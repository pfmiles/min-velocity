...
import com.github.pfmiles.org.apache.velocity.runtime.Renderable;
import com.github.pfmiles.org.apache.velocity.runtime.RuntimeConstants;
import com.github.pfmiles.org.apache.velocity.runtime.directive.Block.Reference;
...
import com.github.pfmiles.org.apache.velocity.runtime.parser.Token;
import com.github.pfmiles.org.apache.velocity.util.ClassUtils;
import com.github.pfmiles.org.apache.velocity.util.introspection.Info;
import com.github.pfmiles.org.apache.velocity.util.introspection.VelMethod;
import com.github.pfmiles.org.apache.velocity.util.introspection.VelPropertySet;
...
public class ASTReference extends SimpleNode {
     ...
     * which basically means that the last syntax of the reference are brackets.
     */
    private ASTIndex astIndex = null;

    /**
     * Indicates if we are using modified escape behavior in strict mode. mainly
     * we allow \$abc -> to render as $abc
     */
    ...
    public Object init(InternalContextAdapter context, Object data) throws TemplateInitException {
        ...
        /*
         * the only thing we can do in init() is getRoot() as that is template
         * based, not context based, so it's thread- and context-safe
         */
        ...
        /**
         * In the case we are referencing a variable with #if($foo) or #if( !
         * $foo) then we allow variables to be undefined and we set strictRef to
         * false so that if the variable is undefined an exception is not
         * thrown.
         */
    ...
    public Object execute(Object o, InternalContextAdapter context) throws MethodInvocationException {
        ...
        /*
         * Iteratively work 'down' (it's flat...) the reference to get the
         * value, but check to make sure that every result along the path is
         * valid. For example:
         * 
         * $hashtable.Customer.Name
         * 
         * The $hashtable may be valid, but there is no key 'Customer' in the
         * hashtable so we want to stop when we find a null value and return the
         * null so the error gets logged.
         */
        ...
        try {
            ...
            for (int i = 0; i < numChildren; i++) {
                if (strictRef && result == null) {
                    ...
                    throw new VelocityException("Attempted to access '" + name + "' on a null value at "
                            + Log.formatFileString(uberInfo.getTemplateName(), +jjtGetChild(i).getLine(), jjtGetChild(i).getColumn()));
                }
    ...
    public boolean render(InternalContextAdapter context, Writer writer) throws IOException, MethodInvocationException {
        ...
        if (escaped && strictEscape) {
             ...
             * bother to retreive the value since we won't use it. And if the
             * var is not defined it will throw an exception. Set value to TRUE
             * to fall through below with simply printing $foo, and not \$foo
             */
            value = Boolean.TRUE;
        } else {
        ...
        if (value != null) {
            if (value instanceof Renderable) {
                ...
                try {
                    ...
                    // We commonly get here when an error occurs within a block
                    // reference.
                    ...
                    log.error("Exception rendering " + ((renderable instanceof Reference) ? "block " : "Renderable ") + rootString + " at "
                            + Log.formatFileString(this));
                    throw e;
                }
            } else if ((value instanceof Reader || value instanceof InputStream)
                    && this.rsvc.getBoolean(RuntimeConstants.STREAM_REFERENCE_RENDERING, false)) {
                // 支持流式渲染，在渲染大数据量的reference时节省内存空间，防止内存过于剧烈地抖动
                BufferedReader br = null;
                if (value instanceof InputStream) {
                    String encoding = (String) this.getRuntimeServices().getProperty(RuntimeConstants.OUTPUT_ENCODING);
                    if (encoding == null)
                        encoding = RuntimeConstants.ENCODING_DEFAULT;
                    br = new BufferedReader(new InputStreamReader((InputStream) value, encoding));
                } else {
                    br = new BufferedReader((Reader) value);
                }
                ImplHelper.flowing(br, writer,
                        this.rsvc.getInt(RuntimeConstants.STREAM_REFERENCE_RENDERING_BUFFER_SIZE, RuntimeConstants.DEFAULT_REFERENCE_RENDERING_BUFFER_SIZE),
                        this.rsvc.getInt(RuntimeConstants.STREAM_REFERENCE_RENDERING_LIMIT, RuntimeConstants.DEFAULT_REFERENCE_RENDERING_LIMIT));
                return true;
            } else {
        ...
        if (value == null || toString == null) {
            if (strictRef) {
                if (referenceType != QUIET_REFERENCE) {
                    ...
                    if (value == null) {
                        throw new VelocityException("Reference " + literal() + " evaluated to null when attempting to render at "
                                + Log.formatFileString(this));
                    } else // toString == null
                    {
                        ...
                        // null.
                        throw new VelocityException("Reference " + literal() + " evaluated to object " + value.getClass().getName()
                                + " whose toString() method returned null at " + Log.formatFileString(this));
                    }
                }
                return true;
            }
            ...
            if (logOnNull && referenceType != QUIET_REFERENCE && log.isDebugEnabled()) {
                log.debug("Null reference [template '" + getTemplateName() + "', line " + this.getLine() + ", column " + this.getColumn() + "] : "
    ...
    public boolean setValue(InternalContextAdapter context, Object value) throws MethodInvocationException {
        ...
        /*
         * The rootOfIntrospection is the object we will retrieve from the
         * Context. This is the base object we will apply reflection to.
         */
        ...
        for (int i = 0; i < numChildren - 1; i++) {
            ...
            if (result == null) {
                if (strictRef) {
                    ...
                    throw new MethodInvocationException("Attempted to access '" + name + "' on a null value", null, name, uberInfo.getTemplateName(),
        ...
        if (astIndex != null) {
            ...
            // reference,
            // something of the form $foo[1] =, or in general any reference that
            // ends with
            // the brackets. This means that we need to call a more general
            // method
            // of the form set(Integer, <something>), or put(Object,
            // <something), where
            // the first parameter is the index value and the second is the LHS
            // of the set.
            ...
            Class[] paramClasses = { params[0] == null ? null : params[0].getClass(), params[1] == null ? null : params[1].getClass() };

            String methodName = "set";
            VelMethod method = ClassUtils.getMethod(methodName, params, paramClasses, result, context, astIndex, false);
            ...
            if (method == null) {
                ...
                // hash
                // form $foo["blaa"], then it may be expensive to first try and
                // fail on 'set'
                // then go to 'put'? The problem is that getMethod will try the
                // cache, then
                // perform introspection on 'result' for 'set'
                methodName = "put";
                method = ClassUtils.getMethod(methodName, params, paramClasses, result, context, astIndex, false);
            }

            if (method == null) {
                // couldn't find set or put method, so bail
                if (strictRef) {
                    throw new VelocityException("Found neither a 'set' or 'put' method with param types '(" + printClass(paramClasses[0]) + ","
                            + printClass(paramClasses[1]) + ")' on class '" + result.getClass().getName() + "' at " + Log.formatFileString(astIndex));
                }
                return false;
            }
            ...
            try {
                ...
                throw new MethodInvocationException("Exception calling method '" + methodName + "(" + printClass(paramClasses[0]) + ","
                        + printClass(paramClasses[1]) + ")' in  " + result.getClass(), e.getCause(), identifier, astIndex.getTemplateName(),
                        astIndex.getLine(), astIndex.getColumn());
            }
         ...
         * ) : 1) ref.setFoo( value ) 2) ref,put("foo", value ) to parallel the
         * get() map introspection
         */
        ...
        try {
            ...
            if (vs == null) {
                if (strictRef) {
                    throw new MethodInvocationException(
                            "Object '" + result.getClass().getName() + "' does not contain property '" + identifier + "'", null, identifier,
            ...
            throw new MethodInvocationException("ASTReference : Invocation of method '" + identifier + "' in  " + result.getClass()
                    + " threw exception " + ite.getTargetException().toString(), ite.getTargetException(), identifier, getTemplateName(),
    ...
    private String getRoot() {
        ...
        /*
         * we have a special case where something like $(\\)*!, where the user
         * want's to see something like $!blargh in the output, but the !
         * prevents it from showing. I think that at this point, this isn't a
         * reference.
         */
        ...
        if (slashbang != -1) {
            ...
            nullString += t.image.substring(start, start + count - 1); // get
                                                                       // the
                                                                       // slashes
        ...
        /*
         * we need to see if this reference is escaped. if so we will clean off
         * the leading \'s and let the regular behavior determine if we should
         * output this as \$foo or $foo later on in render(). Lazyness..
         */
        ...
        /*
         * Look for preceeding stuff like '#' and '$' and snip it off, except
         * for the last $
         */
        ...
        /*
         * if we have extra stuff, loc > 0 ex. '#$foo' so attach that to the
         * prefix.
         */
        if (loc1 > 0) {
        ...
        /*
         * Now it should be clean. Get the literal in case this reference isn't
         * backed by the context at runtime, and then figure out what we are
         * working with.
         */
        ...
        if (t.image.startsWith("$!")) {
            ...
            /*
             * just nip off the '$' so we have the root
             */
    ...
    public Object getVariableValue(Context context, String variable) throws MethodInvocationException {
        ...
        if (strictRef && obj == null) {
            if (!context.containsKey(variable)) {
                ...
                throw new MethodInvocationException("Variable $" + variable + " has not been set", null, identifier, uberInfo.getTemplateName(),
